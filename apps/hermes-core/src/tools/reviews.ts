...
/**
 * Pull reviews off a review page.
 *
...
export const checkReviewsTool: Tool = {
  name: "check_reviews",
  description:
    "Fetch current reviews from the configured review page. Returns each review " +
    "with its rating, author, text, and whether it already has a reply.",
  parameters: {
    type: "object",
    properties: {
      url: { type: "string", description: "Review page. Omit to use the configured one." },
      onlyUnanswered: { type: "boolean", description: "Defaults to true." },
    },
    required: [],
  ...
  async run(args, ctx) {
    ...
    if (!url) {
      throw new Error("No review URL. Set the review page in this agent's configuration.");
    }

    const reviews = await scrapeReviews(url, ctx.signal);
    ctx.log("reviews.fetched", { count: reviews.length });

    const selected =
      args.onlyUnanswered === false ? reviews : reviews.filter((r) => !r.hasReply);

    if (selected.length === 0) {
      return reviews.length === 0
        ? `No reviews could be read from ${url}. The page may render them with ` +
            "JavaScript or require a login. Try the site's public reviews URL."
        : `Found ${reviews.length} reviews, all already answered. Nothing to do.`;
    }

    return (
      `Found ${reviews.length} reviews, ${selected.length} without a reply.\n\n` +
      selected
        .map(
          (r, i) =>
            `--- Review ${i + 1} (id: ${r.id}) ---\n` +
...
function fromJsonLd(html: string): ScrapedReview[] {
  ...
  $('script[type="application/ld+json"]').each((_, el) => {
    ...
    for (const node of flatten(parsed)) collect(node, found);
  });
...
function collect(node: unknown, out: ScrapedReview[]): void {
...
function readRating(rating: unknown): number | null {
  ...
  if (rating && typeof rating === "object") {
    const parsed = Number((rating as Record<string, unknown>).ratingValue);
