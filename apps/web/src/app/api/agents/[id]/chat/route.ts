...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: history } = await admin.from("chat_messages").select("role, content").eq("agent_id", agent.id).eq("user_id", agent.user_id).order("created_at", { ascending: false }).limit(40);
  ...
  await admin.from("chat_messages").insert({ agent_id: agent.id, user_id: agent.user_id, role: "user", content: parsed.data.message });

  try {
    const reply = await respondAsAgent(agent, turns, apiKey);
    await admin.from("chat_messages").insert({ agent_id: agent.id, user_id: agent.user_id, role: "assistant", content: reply });
