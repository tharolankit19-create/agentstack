...
export type FeedbackSession = {
 ...
 usage_snapshot: { outputs?: number; agents?: number; latestOutputAt?: string | null };
 review_note: string | null; improvement_status: string;
 created_at: string; submitted_at: string | null;
};
export function nextFeedbackQuestion(answers: FeedbackAnswer[], used: boolean): string | null {
 const questions = [
  "Think about your last visit. What were you trying to get done, and for what business or project?",
  used
   ? "Walk me through the last task you gave an agent. What did you expect, and what actually happened? A concrete example helps."
