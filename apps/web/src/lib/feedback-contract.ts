...
export function nextFeedbackQuestion(answers: FeedbackAnswer[], used: boolean, context?: FeedbackSession["usage_snapshot"]): string | null {
 const questions = [
  ...
  used
   ...
   : "Where did you stop before getting a useful result? Name the screen, button, setup step, or missing information that blocked you.",
  "What did you have to verify, edit, retry, or do manually after Kryx finished? Give one concrete example.",
  ...
  "If we could change only one thing before your next visit, what should it be? What measurable result would tell you we fixed it?",
 ];
