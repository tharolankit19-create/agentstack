...
/**
 * The in-database scheduler's single configuration row.
 *
 * `secret` is the bearer pg_cron signs the heartbeat with, generated by the
 * migration rather than configured, which is what lets the scheduler work with
 * nothing set by hand. `app_url` is the one field an operator fills in.
 */
