...
export function FloatingHeader({ signedIn }: { signedIn: boolean }) {
  ...
  return (
    <header className="pointer-events-none fixed inset-x-0 top-0 z-50 px-4 pt-3 sm:px-6 sm:pt-4">
      <div
        className={`pointer-events-auto mx-auto flex max-w-5xl items-center justify-between rounded-[22px] border border-line bg-surface/82 px-3.5 text-fg shadow-[0_18px_70px_-34px_rgba(15,23,42,.34)] backdrop-blur-2xl transition-all duration-300 sm:px-4 ${scrolled ? "h-14 translate-y-0.5" : "h-16"}`}
      >
