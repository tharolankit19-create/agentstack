...
const CORE_ROUTES = ["/dashboard", "/dashboard/missions", "/dashboard/agents", "/dashboard/room", "/dashboard/scheduled", "/dashboard/connectors", "/dashboard/usage", "/dashboard/settings"];
      ...
      <div className="flex min-h-0 flex-1 flex-col rounded-[24px] border border-line bg-surface/92 p-3 shadow-[0_24px_70px_-42px_rgba(16,20,32,.45)] backdrop-blur-2xl">
        ...
        <div className="mt-auto space-y-2 pt-5">
          <Link href="/dashboard/usage" className="block rounded-2xl border border-[#4f6bff]/18 bg-[#4f6bff]/[.07] p-3 transition-all hover:-translate-y-0.5 hover:bg-[#4f6bff]/[.11] active:translate-y-0">
            <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#4f6bff]">Work balance</p><p className="mt-1 text-lg font-extrabold text-fg-strong">${dollars.toFixed(2)}</p><p className="text-[10px] text-faint">{balance.toLocaleString()} credits</p></div><span className="rounded-xl bg-fg-strong px-3 py-2 text-xs font-bold text-bg">Buy</span></div>
          </Link>
