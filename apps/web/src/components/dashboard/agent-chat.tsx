...
import { Loader2, Mic, SendHorizonal, Square, Volume2 } from "lucide-react";
...
export function AgentChat({ agentId, paused, history, suggestions }: { agentId: string; paused: boolean; history: ChatMessage[]; suggestions: string[] }) {
  ...
  useEffect(() => () => { streamRef.current?.getTracks().forEach((track) => track.stop()); audioRef.current?.pause(); }, []);
  ...
  async function speak(text: string, index: number) {
    if (speakingIndex === index) { audioRef.current?.pause(); setSpeakingIndex(null); return; }
    setSpeakingIndex(index); setError(null);
    try {
      const response = await fetch("/api/voice/tts", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ text }) });
      if (!response.ok) throw new Error("Voice is temporarily unavailable.");
      const url = URL.createObjectURL(await response.blob());
      const audio = new Audio(url); audioRef.current = audio;
      audio.onended = () => { URL.revokeObjectURL(url); setSpeakingIndex(null); };
      audio.onerror = () => { URL.revokeObjectURL(url); setSpeakingIndex(null); };
      await audio.play();
    } catch (cause) { setSpeakingIndex(null); setError(cause instanceof Error ? cause.message : "Voice is temporarily unavailable."); }
  }
  ...
  return <>
    <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
      ...
      {turns.map((turn, index) => <div key={index} className={cn("flex", turn.role === "user" ? "justify-end" : "justify-start")}><div className={cn("group max-w-[85%] rounded-2xl px-4 py-3", turn.role === "user" ? "bg-accent text-accent-fg" : "border border-line bg-surface-2 text-fg")}>{turn.pending ? <span className="flex items-center gap-2 text-sm text-muted"><Loader2 className="size-4 animate-spin" />Working…</span> : <><p className="whitespace-pre-wrap text-[15px] leading-relaxed">{turn.content}</p>{turn.role === "assistant" ? <div className="mt-2 flex items-center gap-1"><CopyButton value={turn.content} /><button type="button" onClick={() => void speak(turn.content, index)} className="inline-flex size-7 items-center justify-center rounded-md text-muted hover:bg-surface-3 hover:text-fg" aria-label="Speak reply">{speakingIndex === index ? <Square className="size-3.5" /> : <Volume2 className="size-3.5" />}</button></div> : null}</>}</div></div>)}
    ...
    <form onSubmit={(event) => { event.preventDefault(); void send(draft); }} className="mt-4 flex shrink-0 items-end gap-2">
      ...
      <Button type="submit" disabled={pending || !draft.trim()} size="icon" className="size-12" aria-label="Send">{pending ? <Loader2 className="animate-spin" /> : <SendHorizonal />}</Button>
    </form>
  </>;
}
